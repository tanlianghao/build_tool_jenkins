...
const personToken = process.env.PERSON_TOKEN as string;
const xdragonAppProjectId = process.env.XDRAGON_APP_PROJECT_ID as string;
...
server.registerTool('get_gitlab_branch', {
  ...
  try {
    ...
    // 格式化分支信息，包含更多上下文
    const branchList = currentBranch.map((branch, index) => 
      `${index + 1}. ${branch.name}\n  👤 提交者: ${branch.commit.committer_name}`
    ).join('\n');
...
async function getCurrentUserBranch(): Promise<BranchResult[]> {
  let per_page = 100;
  ...
  let totalItems = resHeaders.get('X-Total') ? parseInt(resHeaders.get('X-Total') as string) : 0;
  let totalPages = Math.ceil(totalItems / per_page);
  let promiseAllFetch: Promise<any>[] = [];

  for (let i = 1; i <= totalPages; i++) {
    promiseAllFetch.push(
      fetch(`${getBranchsUrl}?page=${i}&per_page=${per_page}`, {
        method: 'GET',
        headers: {
          'PRIVATE-TOKEN': personToken
        }
...
// 使用jenkins触发构建打包
async function triggerJenkinsBuild(branchName: string, buildType: string) {
  try {
    // 配置信息
